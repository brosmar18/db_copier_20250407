...
class LoginPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # Page Title
        label_title = ttk.Label(content_frame, text="PostgreSQL Login", font=("Helvetica", 16, "bold"))
        ...
        # Host field
        ttk.Label(content_frame, text="Host:").grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.entry_host = ttk.Entry(content_frame, textvariable=self.host_var)
        self.entry_host.grid(row=1, column=1, padx=10, pady=5)

        # Port field
        ttk.Label(content_frame, text="Port:").grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.entry_port = ttk.Entry(content_frame, textvariable=self.port_var)
        self.entry_port.grid(row=2, column=1, padx=10, pady=5)

        # Username field
        ttk.Label(content_frame, text="Username:").grid(row=3, column=0, padx=10, pady=5, sticky="w")
        ...
        ttk.Label(content_frame, text="Password:").grid(row=4, column=0, padx=10, pady=5, sticky="w")
        self.entry_password = ttk.Entry(content_frame, textvariable=self.password_var, show="*")
        ...
        # Connect button
        button_connect = ttk.Button(content_frame, text="Connect", command=self.attempt_login)
