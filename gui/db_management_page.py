...
class DBManagementPage(ttk.Frame):
    ...
    def delete_selected_database(self):
        """Delete selected database(s) with confirmation."""
        selected = self.db_tree.selection()
        if not selected:
        ...
        def delete_database():
            ...
            if errors:
                error_msg = "Failed to delete the following databases:\n\n" + "\n".join(
                    errors
                )
                self.after(0, lambda: messagebox.showerror("Deletion Error", error_msg))
            else:
                self.after(
                    0,
                    lambda: messagebox.showinfo(
                        "Success", "Database(s) deleted successfully."
                    ),
                )

            # Refresh the database list on the main thread
